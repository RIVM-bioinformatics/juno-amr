...
inlude: "bin/rules/makeResfinderSummary.smk"
...
rule all:
    ...
    input: 
        ...
        expand(OUT + "/results_per_sample/{sample}/ResFinder_results.txt", sample=config["samples_fastq_r1"]),
        expand(OUT + "/results_per_sample/{sample}/pheno_table.txt", sample=config["samples_fastq_r1"]),
        expand(genes_summary = OUT + "summary/summary_amr_genes.csv")
